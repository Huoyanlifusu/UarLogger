...
// Feature Points Vertex Function
vertex FragIn featurePointVertexShader(VtxIn vtx [[stage_in]], constant float4x4& mvp [[buffer(kBufferIndexMVP)]] )
{
    FragIn ret;
    ret.position = mvp * float4(vtx.position, 1.0f);
    //ret.color    = float4(vtx.position, 1.0f);
    ret.color = float4( 1.0f, 0.0f, 0.0f, 1.0f );
    ret.point_size = 10.0f;
    
    return ret;
}

// Feature Points Fragment Function
fragment float4 featurePointFragmentShader(FragIn in [[stage_in]])
{
    return in.color;
}
