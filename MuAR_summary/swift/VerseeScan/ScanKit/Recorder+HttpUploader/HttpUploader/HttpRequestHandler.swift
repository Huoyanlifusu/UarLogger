...
class HttpRequestHandler: NSObject {
    ...
    private let origin: String = UploadConfig.httpOrigin
    ...
    func uploadAllFilesOneByOne(fileUrls: [URL]) {
        ...
        let uploadTask = session.uploadTask(with: uploadRequest, fromFile: currentFileUrl, completionHandler: {
            ...
            guard let httpUrlResponse = response as? HTTPURLResponse,
                  (200...299).contains(httpUrlResponse.statusCode) else {
                UploadConfig.errorCount += 1
                if UploadConfig.errorCount >= 3 {
                    print("Upload failed three times after multiple attempts, will no longer attempt to upload \(currentFileUrl.absoluteString)")
                    print(response.debugDescription)
                    self.httpRequestHandlerDelegate.didCompletedUploadWithError()
                    UploadConfig.errorCount = 0
                    return
                }
                print("The upload failed for the \(UploadConfig.errorCount) time, currently attempting to retry.")
                self.httpRequestHandlerDelegate.didReceiveReuploadRequest(fileUrls)
                return
            }
