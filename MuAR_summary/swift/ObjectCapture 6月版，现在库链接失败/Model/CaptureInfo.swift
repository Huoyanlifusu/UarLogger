...
struct CaptureInfo: Identifiable {
        ...
        let future = Future<FileExistence, CaptureInfo.Error> { promise in
            CaptureInfo.loaderQueue.async {
                guard CaptureInfo.doesDirectoryExist(url: captureDir) else {
                    Logger.shared.debugPrint("checkFilesExist: can't find dir \(captureDir)!")
                ...
                do {
                    ...
                    Logger.shared.debugPrint("checkFilesExist: error \(String(describing: error))!")
                    promise(.failure(CaptureInfo.Error.noSuchDirectory(captureDir)))
                }
            }
        }
    ...
    private func deleteHelper(delete: URL, fileType: String) {
        do {
            Logger.shared.debugPrint("Deleting \(fileType) at \"\(delete.path)\"...")
            try FileManager.default.removeItem(atPath: delete.path)
            Logger.shared.debugPrint("... deleted \"\(delete.path)\".")
        } catch {
            Logger.shared.debugPrint("Error 107: Can't delete \(fileType) \"\(delete.path)\" error=\(String(describing: error))")
        }
    }
    ...
    static func imageUrl(in captureDir: URL, id: UInt32) -> URL {
        return captureDir.appendingPathComponent(photoIdString(for: id).appending("_image"), conformingTo: .jpeg)
    }
    ...
    static func depthUrl(in captureDir: URL, id: UInt32) -> URL {
        return captureDir.appendingPathComponent(photoIdString(for: id).appending("_depth"), conformingTo: .png)
    }
