...
for i in range(len(pk_ni)):
    x_cam.append(pk_arkit[i][0]-x_groundTruth)
    y_cam.append(pk_arkit[i][1]-y_groundTruth)
    z_cam.append(pk_arkit[i][2]-z_groundTruth)
    x_ni.append(pk_ni[i][0]-x_groundTruth)
    y_ni.append(pk_ni[i][1]-y_groundTruth)
    z_ni.append(pk_ni[i][2]-z_groundTruth)
    x1.append(i)
fig = plt.figure()
...
k = 0
# 常数参数初始化
rho = 0.0002 # 论文里未提及 应该是一个控制beta是否更新的阈值
zeta = 0.0001
epsilon = 0.001
Len = 100 # 单次更新次数
m = 40 # 滑动窗口的大小
...
newz = []
for i in range(m):
    newx.append(pk_arkit[i][0]*beta_init[0][0]-x_groundTruth)
    newy.append(pk_arkit[1][1]*beta_init[1][0]-y_groundTruth)
    newz.append(pk_arkit[i][2]*beta_init[2][0]-z_groundTruth)
for i in range(m, len(pk_ni)):
    newx.append(pk_arkit[i][0]*sx[i-m]-x_groundTruth)
    newy.append(pk_arkit[i][1]*sy[i-m]-y_groundTruth)
    newz.append(pk_arkit[i][2]*sz[i-m]-z_groundTruth)
x2 = [i for i in range(len(newx))]
xg =[x_groundTruth] * len(x1)
plt.plot(x1, x_cam, color = 'r', label = 'arkit x err with gt')
plt.plot(x1, x_ni, color = 'g', label = 'ni x err with gt')
plt.plot(x2, newx, color = 'b', label = 'lm err with gt')
plt.xlabel("frame")
plt.ylabel("m")
plt.legend()
plt.show()
...
plt.plot(x1, y_cam, color = 'r', label = 'arkit y err with gt')
plt.plot(x1, y_ni, color = 'g', label = 'ni y err with gt')
plt.plot(y2, newy, color = 'b', label = 'lm err with gt')
plt.xlabel("frame")
plt.ylabel("m")
plt.legend()
plt.show()
...
plt.plot(x1, z_cam, color = 'r', label = 'arkit z err with gt')
plt.plot(x1, z_ni, color = 'g', label = 'ni z err with gt')
plt.plot(z2, newz, color = 'b', label = 'lm err with gt')
plt.xlabel("frame")
plt.ylabel("m")
plt.legend()
plt.show()