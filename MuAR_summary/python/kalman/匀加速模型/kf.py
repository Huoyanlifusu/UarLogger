...
std = [np.std(xv), np.var(yv), np.std(zv, ddof=1),
       np.var(xu), np.var(yu), np.var(zu, ddof=1)]
Wn = np.diag([1, 1, 1, 100, 5, 100])
Wk = np.diag(std) + Wn
# 状态方程噪声
...
for k in range(frames):
    ...
    # Kk = np.dot(Ck, np.dot(Pk_pre, np.transpose(Ck)), np.linalg.inv(Sk))

    left = np.dot(Ck, np.dot(Pk_pre, np.transpose(Ck)))
    right = np.linalg.inv(np.dot(Ck, np.dot(Pk_pre, np.transpose(Ck))) + Wk)
    Kk = left@right
