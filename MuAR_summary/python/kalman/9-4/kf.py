...
for k in range(frames):
    ...
    if k > 500:
        for t in range(50):
            maxnum = max(rawx[-1-t], maxnum)
            minnum = min(rawx[-1-t], minnum)
            avg_raw += rawx[-1-t]
        avg_raw /= 50.0
        if abs(maxnum - minnum) > 0.01:
            kalmanx.append(avg_raw)
        else:
            kalmanx.append(rawx[-1])
    else:
        kalmanx.append(rawx[-1])

    if k > 500 and abs(xk[1][0] - kalmany[-1]) > 0.002:
        kalmany.append(kalmany[-1])
    else:
        kalmany.append(xk[1][0])

    if k > 500 and abs(xk[2][0] - kalmanz[-1]) > 0.002:
        kalmanz.append(kalmanz[-1])
    else:
